...
CBusClient.prototype.lockState = function (netId, callback) {
	const cmd = this._buildSetCommandString(netId, 'on');
	this._sendMessage(cmd, callback);
};

CBusClient.prototype.unlockState = function (netId, callback) {
	const cmd = this._buildSetCommandString(netId, 'off');
	this._sendMessage(cmd, callback);
};
